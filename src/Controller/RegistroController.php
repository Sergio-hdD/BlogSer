...
class RegistroController extends AbstractController
{
    ...
    public function index(Request $request, UserPasswordEncoderInterface $passwordEncoder): Response //recibo la solicitud por parametro, asigno en una variable lo que importé de la librería para cifrar la password  
    {
        ...
        if($form->isSubmitted() && $form->isValid()){//si el formulario fue enviado y el formulario es válido
            ...
            if($em->getRepository(User::class)->findOneBy([ 'email' => $form['email']->getData() ])){ //Si ya hay un usuario con el mismo email
                $this->addFlash('mensaje_error',User::EMAIL_DUPLICADO);
            ...
            return $this->redirectToRoute('app_login');//Redirecciona a una ruta a través del "name" del "@Route...", este caso quiero que redireccion a esta misma función "index()", pero puede redirgir a una función que esté en otro lugar
        }
